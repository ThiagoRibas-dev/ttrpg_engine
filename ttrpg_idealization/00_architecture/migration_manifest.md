...
| `03_core_baseline_system/01_resolution_engine.md` §5–6 | `01_framework/01_core_engine/03_boons_banes_and_die_steps.md` | Canonical specification | **Migrated 2026-09-19 (Batch 3, DEC-109)** | Final-pool timing, cancellation, five typed-stacking categories with same-type/different-type/Untyped/same-source rules, Boon adds a die, Bane removes one with one-die-pool Step-Down and `d4` automatic-failure chain, Die Step-Up/Down ordering before Boons/Banes, `d12` ceiling with Exert exception, `d4` floor rules. Archived glossary Boon/Bane entries cross-checked (consistent). Open definitions: notation collision (`X` as Bane marker vs. `2dX` die-size variable) flagged, not changed. Registry node `core.boons-banes-and-die-steps` + `uses` edge to `core.checks-and-pools`. |
| `03_core_baseline_system/01_resolution_engine.md` §7–8 | `01_framework/01_core_engine/04_opposed_checks_and_automatic_successes.md` | Canonical specification | Planned — **Batch 4, next** | Opposed contests, defender tie-wins, Automatic Successes. |
