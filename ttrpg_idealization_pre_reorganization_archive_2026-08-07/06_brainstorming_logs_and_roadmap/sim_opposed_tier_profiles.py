...
def distribution(sides,pool):
 ...
 return keys, [pairs[i][1] for i in range(len(pairs))], prefix

cache={label:distribution(s,p) for label,s,p in PROFILES}

def compare(a,b):
 ak,aw,ap=cache[a]; bk,bw,bp=cache[b]
 wins=ties=0.0
 for key,w in zip(ak,aw):
  i=bisect_left(bk,key)
  less=bp[i-1] if i else 0.0
  eq=(bp[i] if i==0 else bp[i]-bp[i-1]) if i<len(bp) else 0.0
