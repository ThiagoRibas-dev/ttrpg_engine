...
### 8.3 Relationship vocabulary — pending final terminology decision

The relationship vocabulary is deliberately not locked yet. The terminology matrix proposes the following TTRPG-facing registry terms, replacing software-style terms such as `extends`, `implements`, and `constrains`:
...
This table is a proposal pending explicit approval.
...
1. Approve, revise, or reject the proposed registry relationship vocabulary in Section 8.3.
2. Decide which existing documents should become the initial Framework Architecture Map, versus being preserved only in the sibling archive.
3. Decide which existing major decisions require a full Decision Record during early migration.
4. Decide which minimal framework subsection is stable enough to migrate first after the architecture skeleton is created.
5. Test and approve the exact stable-anchor syntax in the target Markdown renderer before broad migration.
